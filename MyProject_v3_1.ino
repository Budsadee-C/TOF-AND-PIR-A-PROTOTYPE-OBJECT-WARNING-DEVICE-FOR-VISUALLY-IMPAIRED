...
void loop() {
  ...
  Serial.println(" ");
  if(measure.RangeStatus != 4 && (tof_front>=400 && tof_front<=1300)) // detect from front but dont know what is it
  {
    status_tof = true;
  }
  else
  ...
  if(status_pir==true && status_tof==true) // found alive
  {
    ...
    for(int dutyCycle = 0; dutyCycle <= 55; dutyCycle+=5) // 35,50 to use
    {   
  ...
  else if(status_pir==false && status_tof==true) // found somthing not alive
  {
    ...
    for(int dutyCycle = 0; dutyCycle <= 50; dutyCycle+=5) // 35,50 to use
    {   
      ledcWrite(ledChannel, dutyCycle);
      delay(20);  // 50 ,100 , 250 
    }
  ...
  if (measure.RangeStatus != 4 && (tof_top>=400 && tof_top<=1400)) // tof_top dectect found something
  {  
   Serial.print("T : "); 
   Serial.print(tof_top);
   Serial.println(" ");
    for(int dutyCycle = 0; dutyCycle <= 45; dutyCycle+=5) // 35,50 to use
    {   
